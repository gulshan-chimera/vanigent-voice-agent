...
// ---------------------------------------------------------------------------
// MICROSOFT GRAPH AUTH + SHAREPOINT HELPERS
// ---------------------------------------------------------------------------
...
async function listFilesInFolder(
  ...
  if (data.error) {
    console.warn(
      `  [WARN] Could not list "${folderName}": ${data.error.message}`
    );
    return [];
  }
...
async function clearIndex(
  ...
  if (failed.length > 0) {
    ...
    console.log(
      `[CLEAR] ✓ All ${existingIds.length} old document(s) removed.`
    );
  }
}
    ...
    for (const file of files) {
      ...
      try {
        ...
        if (!rawText || rawText.trim().length === 0) {
          console.log(
            `  [SKIP] No text extracted — PDF may be image-based`
          );
          totalSkipped++;
          continue;
        }
...
async function sync(): Promise<void> {
  ...
  const { totalIndexed, totalSkipped } = await reindex(
    searchClient,
    token,
    driveId
  );
